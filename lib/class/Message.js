...
class Message extends Base {
	...
	async send(content, options = {}) {
		let jid = options.jid || this.jid;
		let type = options.type;
		let messageContent;
		if (typeof content === "string" && !options.jid) {
			jid = this.jid;
			type = "text";
			messageContent = { text: content };
		} else if (typeof content === "object" && content !== null) {
			jid = content.jid || this.jid;
			type = "text";
			messageContent = { text: content.text || "" };
		} else {
			if (typeof content === "string") {
				if (this._isUrl(content)) {
					type = type || (await this._detectTypeFromUrl(content));
					messageContent = { url: content };
				} else {
					type = "text";
					messageContent = { text: content };
				}
			} else if (Buffer.isBuffer(content)) {
				type = type || (await this._detectTypeFromBuffer(content));
				messageContent = content;
			} else if (Array.isArray(content)) {
				type = "buttons";
				messageContent = { buttons: content };
			} else if (typeof content === "object" && content !== null) {
				if ("latitude" in content && "longitude" in content) {
					type = "location";
					messageContent = { degreesLatitude: content.latitude, degreesLongitude: content.longitude };
				} else if ("displayName" in content && "vcard" in content) {
					type = "contact";
					messageContent = { displayName: content.displayName, vcard: content.vcard };
				} else {
					type = type || this._detectTypeFromObject(content);
					messageContent = content;
				}
			} else {
		...
		if (options.caption && ["image", "video", "document"].includes(type)) {
			message[type] = { ...message[type], caption: options.caption };
		}

		if (type === "document" && options.fileName) {
			message.document = { ...message.document, fileName: options.fileName };
		}
		...
		Object.keys(options).forEach(key => {
			if (!["jid", "type", "caption", "fileName", "quoted"].includes(key)) {
	...
	_detectTypeFromObject(obj) {
		...
		const validTypes = ["image", "video", "audio", "document", "sticker", "buttons", "location", "contact"];
