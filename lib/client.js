...
const clientMsgs = conn => async m => {
	...
	if (config.AUTO_BIO === true) {
		console.log("Setting up auto bio update");
		const cron = require("node-cron");
		const {getJson} = require("./src/functions");
		const moment = require("moment-timezone");
		async function updateStatus() {
			try {
				const res = await getJson(`https://api.popcat.xyz/pickuplines`);
				const txt = res.pickupline;
				const time = moment().tz(config.TIME_ZONE).format("HH:mm");
				const status = `${txt} \n⏰TIME: ${time} \n🚀FX-MD`;
				await conn.updateProfileStatus(status);
				console.log("Bio updated successfully:", status);
			} catch (error) {
				console.error("Error updating bio:", error);
			}
		}
		cron.schedule("*/1 * * * *", updateStatus, {
			scheduled: true,
